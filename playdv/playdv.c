...
#define DV_PLAYER_OPT_RENDERER_NAME   11
#define DV_PLAYER_OPT_SINGLE_STEP     12
#define DV_PLAYER_NUM_OPTS            13
...
typedef struct {
  ...
  struct stat       statbuf;
  struct timeval    tv[3];
  int               arg_disable_audio_output;
  int               arg_disable_audio_decode;
  int               arg_disable_video_output;
  int               arg_disable_video_decode;
  int               arg_format;
  int               arg_size;
  int               arg_code;
  int               no_mmap;
...
dv_player_new(void)
{
  ...
  if(!(result->decoder = dv_decoder_new(TRUE, 1, 1))) goto no_decoder;
  result->arg_loop_count = 1;
  result->arg_renderer_name = "YUY2";
  result->arg_single_step = 0;
  ...
  result->option_table[DV_PLAYER_OPT_DISABLE_AUDIO] = (struct poptOption) {
    longName: "disable-audio-output",
    arg:      &result->arg_disable_audio_output,
    descrip:  "skip audio output",
  }; /* disable audio */

  result->option_table[DV_PLAYER_OPT_DISABLE_AUDIO] = (struct poptOption) {
    longName: "disable-audio-decode",
    arg:      &result->arg_disable_audio_decode,
    descrip:  "skip audio decoding",
  }; /* disable audio */

  result->option_table[DV_PLAYER_OPT_DISABLE_VIDEO] = (struct poptOption) {
    longName: "disable-video-output",
    descrip:  "skip video output",
    arg:      &result->arg_disable_video_output,
  }; /* disable video */

  result->option_table[DV_PLAYER_OPT_DISABLE_VIDEO] = (struct poptOption) {
    longName: "disable-video-decode",
    descrip:  "skip video decoding",
    arg:      &result->arg_disable_video_decode,
  }; /* disable video */
  ...
  result->option_table[DV_PLAYER_OPT_RENDERER_NAME] = (struct poptOption) {
    longName:   "renderer",
    argInfo:    POPT_ARG_STRING,
    arg:        &result->arg_renderer_name,
    argDescrip: "renderer name",
    descrip:    "select the render function",
  }; /* loop count */

  result->option_table[DV_PLAYER_OPT_SINGLE_STEP] = (struct poptOption) {
    longName:   "step",
    argInfo:    POPT_ARG_NONE,
    arg:        &result->arg_single_step,
    argDescrip: "single step",
    descrip:    "single step execution <enter>",
  }; /* loop count */
 ...
 */
void
mmap_unaligned(int fd, int no_mmap, off_t offset, size_t length,
               dv_mmap_region_t *mmap_region)
{
    size_t real_length;
    off_t  real_offset;
 ...
 */
int
munmap_unaligned(dv_mmap_region_t *mmap_region, int no_mmap)
{
  if (no_mmap) {
    return 0;
  } else {
...
//  dv_select_renderer (dv_player -> decoder, format, attr);
  ...
  dv_display_init (dv_player->display, &argc, &argv,
                   dv_player->decoder->width, dv_player->decoder->height,
                   dv_player->decoder->sampling,
                   dv_player->decoder->current_renderer->four_cc,
                   dv_player->decoder->current_renderer->attr,
                   "playdv", "playdv");

  dv_player->arg_disable_audio_output =
    dv_player->arg_disable_audio_output ||
      (!dv_oss_init(dv_player->decoder->audio, dv_player->oss));

  for(i=0; i < 4; i++) {
    if(!(audio_buffers[i] = malloc(DV_AUDIO_MAX_SAMPLES*sizeof(short)))) goto no_mem;
  } /* if */
...
restart:
  ...
  for(offset=0;
    ...
    if(dv_decode_full_audio(dv_player->decoder, dv_player->mmap_region.data_start, audio_buffers)) {
      if(!dv_player->arg_disable_audio_output) {
        dv_oss_play(dv_player->decoder->audio, dv_player->oss, audio_buffers);
      } /* if */
    } /* if */

    if(!dv_player->arg_disable_video_decode) {
#if 0
      if (dv_format_wide (dv_player->decoder))
        fprintf (stderr, "format 16:9\n");
      if (dv_format_normal (dv_player->decoder))
      ...
      /* Display */
      if(!dv_player->arg_disable_video_output) {
